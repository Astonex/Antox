...
import im.tox.antox.utils.{Constants, FriendInfo, FriendRequest, LeftPaneItem}
import im.tox.jtoxcore.{FriendExistsException, ToxException}
import rx.lang.scala.schedulers.{AndroidMainThreadScheduler, IOScheduler}
import rx.lang.scala.{Observable, Subscription}

//remove if not needed._
...
class ContactsFragment extends Fragment {
  ...
  def compareNames(a: FriendInfo, b: FriendInfo): Boolean = {
    if (a.alias != "") {
      if (b.alias != "") a.alias.toUpperCase.compareTo(b.alias.toUpperCase) == -1
      else a.alias.toUpperCase.compareTo(b.friendName.toUpperCase) == -1
    } else {
      if (b.alias != "") a.friendName.toUpperCase.compareTo(b.alias.toUpperCase) == -1
      else a.friendName.toUpperCase.compareTo(b.friendName.toUpperCase) == -1
    }
  }
